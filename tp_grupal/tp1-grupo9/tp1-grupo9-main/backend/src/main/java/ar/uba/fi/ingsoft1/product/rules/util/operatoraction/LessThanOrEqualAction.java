...
import ar.uba.fi.ingsoft1.product.rules.exceptions.actionexceptions.LessThanOrEqualActionException;
import ar.uba.fi.ingsoft1.product.rules.util.OperatorUtils;

public class LessThanOrEqualAction implements OperatorAction {
    @Override
        ...
        try {
            ...
            if (min.isPresent()) {
                if (OperatorUtils.compareNumbers(valueToCompare, min.get(), (a, b) -> a <= b)) {
                    ...
                    throw new LessThanOrEqualActionException(String.format("The value %.2f is not less than or equal (<=) to %.2f", valueToPrint, min.get().doubleValue()));
                }
            }
