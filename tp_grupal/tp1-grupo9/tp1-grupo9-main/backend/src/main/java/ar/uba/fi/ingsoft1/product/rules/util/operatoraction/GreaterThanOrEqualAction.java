...
import ar.uba.fi.ingsoft1.product.rules.exceptions.actionexceptions.GreaterThanOrEqualActionException;
import ar.uba.fi.ingsoft1.product.rules.util.OperatorUtils;

public class GreaterThanOrEqualAction implements OperatorAction {
    @Override
        ...
        try {
            ...
            if (max.isPresent()) {
                if (OperatorUtils.compareNumbers(valueToCompare, max.get(), (a, b) -> a >= b)) {
                    ...
                    throw new GreaterThanOrEqualActionException(String.format("The value %.2f is not greater than or equal (>=) to %.2f", valueToPrint, max.get().doubleValue()));
                }
            }
